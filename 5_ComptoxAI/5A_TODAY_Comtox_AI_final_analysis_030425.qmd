...
# Get info about source nodes, target nodes, and overlapping genes
...
print(Counter(leidenalg_community_4))
print(Counter(leidenalg_community))
```

```{python}
# Get information for overlapping genes
overlapping_gene_info = []
for gene in overlapping_gene_list:
    try:
        gene_vertex = h.vs.find(name=gene)
        gene_cluster = partition.membership[gene_vertex.index]
        gene_cluster_size = Counter(partition.membership)[gene_cluster]
        overlapping_gene_info.append((gene, gene_cluster, gene_cluster_size))
    except ValueError:
        overlapping_gene_info.append((gene, None, None))  # Handle case where gene is not found

# Print overlapping gene information
for gene, cluster, cluster_size in overlapping_gene_info:
    if cluster is not None:
        print(f'Gene: {gene}, Cluster: {cluster}, Cluster Size: {cluster_size}')
    else:
...
# Add leidenalg_community_4 information to NetworkX graph
...
#     print(f"Node: {node}, z: {data.get('z')}, clust_type: {data.get('clust_type')}, clust: {data.get('leidenalg_community_4')}")
```
...
# Check to make sure that the order of nodes is the same
node_ids = list(G_trim.nodes)
df_embeddings = pd.DataFrame(node_ids, columns=['gene'])
Counter(df_embeddings["gene"] == partition.graph.vs['geneSymbol'])
# Assuming h.vs['geneSymbol'] and G_trim.nodes are both lists
gene_symbols = h.vs['geneSymbol']
nodes = G_trim.nodes
...
df_embeddings['leidenalg_community_4'] = leidenalg_community_4
df_embeddings['leidenalg_community_8'] = leidenalg_community_8
...
```{python}
#| label:  Cross tabs
cross_table = pd.crosstab(df_merged['sig_overall'], df_merged['leidenalg_community_4'])
# Convert counts to percentages
# cross_table_percent = cross_table.div(cross_table.sum(axis=1), axis=0) * 100
cross_table
```

# 5. Modify Node Gravity for Gephi figure

```{python}
#| label:  setg up graph for gephi multigravity ForceAtlas2 layout
for idx, community in enumerate(leidenalg_community_8):
    nx_node = mapping[idx]
    if community == 0:
        G_trim.nodes[nx_node]['gravity_x'] = 2000
        G_trim.nodes[nx_node]['gravity_y'] = 0
    elif community == 1:
        G_trim.nodes[nx_node]['gravity_x'] = 0
        G_trim.nodes[nx_node]['gravity_y'] = -2000
    elif community == 2:
        G_trim.nodes[nx_node]['gravity_x'] = -20000
        G_trim.nodes[nx_node]['gravity_y'] = 0
    elif community == 3:
        G_trim.nodes[nx_node]['gravity_x'] = -1000
        G_trim.nodes[nx_node]['gravity_y'] = -1000
    elif community == 4:
        G_trim.nodes[nx_node]['gravity_x'] = 0
        G_trim.nodes[nx_node]['gravity_y'] = -100
    elif community == 5:
        G_trim.nodes[nx_node]['gravity_x'] = 0
        G_trim.nodes[nx_node]['gravity_y'] = 1000
    elif community == 6:
...
```{python}
#| label:  save graph
file_path = dir_res / 'ComptoxAI' / 'PFAS_prot_in_vitro_sig_101124.graphml'
